 ...
 * @method string datepicker(string $name, string $value, array $options = []) static 日期选择组件(https://www.cnblogs.com/HKKILL/p/12209810.html)
...
EOD;
    ...
    /**
     * 日期时间选择器 (属性参考下面链接，可在options传入data-format来覆盖默认的格式化)
     * 
     * @param string $name
     * @param mixed  $value
     * @param array  $options
...
EOD;
    ...
    public function images($name = null, $value, $hiden_upload = false, $hiden_select = false)
    {
        ...
        $html = <<<EOL
            ...
            <div class="upload-preview-list images-sort-list">
        EOL;
        if (count($value) > 0) {
